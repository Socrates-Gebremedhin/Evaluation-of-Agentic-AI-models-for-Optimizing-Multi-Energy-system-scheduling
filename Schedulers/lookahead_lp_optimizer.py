import io
import contextlib
def optimize_24h(date_time_index, frequency, components, elec_demand, gas_demand, pv_generated,gas_price,grid_price,feed_in_price):
    """
    ...
    for i in range(W):
        ...
        cons += [
            ...
            boiler_hd[i] + boiler_buff[i] <= boiler.rated_output_heat * boiler_on[i]
        ]
        ...
        cons += [
            ...
            chp_in[i] <= chp.rated_input_heat * chp_on[i]
        ]
    ...
    objective = cp.Minimize(chp_fuel_cost + boiler_cost + grid_cost - feed_in_revenue + unmet_penalty)
    ...
    # all_solvers = [cp.CLARABEL,cp.CVXOPT, cp.ECOS, cp.GLPK, cp.GLPK_MI, cp.OSQP, cp.SCIPY, cp.SCS,cp.ECOS_BB]
    ...
    out_df = pd.DataFrame({
        ...
        "boiler_out_total": boiler_arr + safe_arr(boiler_buff, W),
